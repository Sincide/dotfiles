    ...
    <box
      ...
      setup={self => {
        ...
        const cpuTimer = cpuUsage.poll(2000, () => {
          try {
            const cpuInfo = Utils.exec("grep 'cpu ' /proc/stat | awk '{usage=($2+$4)*100/($2+$3+$4+$5)} END {print usage}'")
        ...
        const memTimer = memoryUsage.poll(2000, () => {
          try {
            const memInfo = Utils.exec('free -m | grep "^Mem:" | awk \'{print $3 " " $2}\'')
        ...
        const tempTimer = temperature.poll(5000, () => {
          try {
            const temp = Utils.exec("sensors | grep 'Package id 0:' | awk '{print $4}' | tr -d '+°C'")
