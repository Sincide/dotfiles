...
func runTUI(items []Item, prompt, mode string) *Item {
	if len(items) == 0 {
	...
	for {
		...
		// CORRECTED: Combine list and preview using absolute cursor positioning.
		for i := 0; i < maxItems; i++ {
			screenRow := i + 3 // Header takes 2 lines, so content starts at row 3

			// Boundary check for listLines
			if i < len(listLines) {
			...
			if showPreview && i < len(previewLines) {
				screenBuf.WriteString(fmt.Sprintf("\033[%d;%dH", screenRow, listWidth+2)) // Move to preview column
...
func main() {
	...
	selectedItem := runTUI(items, prompt, mode)
