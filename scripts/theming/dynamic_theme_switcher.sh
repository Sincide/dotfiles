...
# Theme mappings - Updated with beautiful modern themes for 2025
declare -A THEMES=(
    # Space wallpapers - Dark, sleek, futuristic themes
    ["space_gtk"]="Graphite-Dark"
...
EOF
    ...
    # Reload Hyprland to apply cursor changes
    if command -v hyprctl >/dev/null 2>&1 && pgrep -x Hyprland >/dev/null; then
        echo "🔄 Reloading Hyprland for cursor theme..."
        hyprctl reload >/dev/null 2>&1 || echo "⚠️  Failed to reload Hyprland"
    fi
