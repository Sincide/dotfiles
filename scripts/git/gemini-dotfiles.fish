...
# ============================================================================
# GEMINI DOTFILES MANAGER - FISH EDITION
# ============================================================================
# Smart dotfiles management with Gemini-powered commit messages
# Author: Fish Script
# Version: 1.0
# 
# Features:
...
# - Fallback commit messages when Gemini fails
#
# Requirements:
# - Fish shell
# - Git repository
# - Gemini CLI (optional, for AI features)
#
# Usage:
#   ./gemini-dotfiles.fish           # Interactive menu
#   ./gemini-dotfiles.fish sync      # Quick sync with AI
#   ./gemini-dotfiles.fish status    # Show repo status
# ============================================================================
...
function check_gemini_availability
    if not command -v gemini >/dev/null 2>&1
        return 1
    end
    
    # Test Gemini basic functionality
    if not gemini --version >/dev/null 2>&1
        return 1
    end
...
# Generate AI-powered commit message using Gemini
function generate_ai_commit
    ...
    if not check_gemini_availability
        warn "Gemini CLI not available -- please ensure 'gemini' is installed and in your PATH" >&2
        # Continue, attempting to invoke Gemini anyway
    end
    
    info "🤖 Analyzing changes with Gemini..." >&2
    ...
    set ai_output (echo $prompt | gemini 2>/dev/null)
    
    # Gemini usage tracking removed
    
    debug "Raw Gemini output: '$ai_output'" >&2
    
    # Clean up the output - remove any quotes, backticks, or extra formatting
    set ai_output (echo $ai_output | sed 's/^[\"'''''`]*//; s/[\"'''''`]*$//')
    
    debug "Processed Gemini output: '$ai_output'" >&2
...
function test_gemini
    info "Testing Gemini commit generation..."
    
    if not check_gemini_availability
        error "Gemini CLI not available"
        error "Please install the Gemini CLI and make sure it's in your PATH"
        return 1
    end
    ...
    set test_message (echo "Generate a commit message for: Updated fish shell configuration" | gemini 2>/dev/null)
...
# Debug Gemini generation with detailed output
function debug_gemini
    set -x DEBUG 1  # Enable debug output
    
    info "Debugging Gemini generation..."
    ...
    info "Gemini CLI is available"
    
    # Test Gemini directly first
    info "Testing basic Gemini functionality..."
    set test_response (echo "Say hello briefly" | gemini 2>&1)
    set test_exit $status
    
    if test $test_exit -ne 0
        error "Basic Gemini test failed with exit code $test_exit"
        error "Output: $test_response"
        return 1
    else
        success "Basic Gemini test passed: $test_response"
    end
...
function show_interactive_menu
    while true
        ...
        echo
        echo "  1️⃣  Sync (Gemini-powered commit & push)"
        echo "  2️⃣  Status (repository overview)"
        echo "  3️⃣  Diff (show changes)"
        echo "  4️⃣  Gemini Test (test commit generation)"
        echo "  5️⃣  Gemini Debug (detailed diagnostics)"
        ...
        switch $choice
            case 1
                read -P "Custom commit message (or Enter for Gemini): " custom_msg
...
function show_help
    ...
    echo "  ./gemini-dotfiles.fish           - Interactive menu (default)"
    echo "  ./gemini-dotfiles.fish [command] - Direct command"
    echo
    set_color yellow; echo "Commands:"; set_color normal
    echo "  sync [message]    - Smart sync with Gemini commits"
    echo "  status           - Show repository status"
    echo "  diff             - Show changes"
    echo "  gemini-test      - Test Gemini commit generation"
    echo "  gemini-debug     - Debug Gemini generation with details"
    echo "  toggle-remote    - Toggle remote between SSH and HTTPS"
    echo "  help             - Show this help"
    
    echo
    set_color yellow; echo "Features:"; set_color normal
    echo "  🤖 Gemini-powered commit messages"
    echo "  🔄 Smart git sync"
    ...
    echo "  ./gemini-dotfiles.fish                    # Interactive menu"
    echo "  ./gemini-dotfiles.fish sync               # Quick Gemini sync"
    echo "  ./gemini-dotfiles.fish sync \"fix bug\"     # Custom message"
    echo "  ./gemini-dotfiles.fish status             # Check status"
end

# Main function
function main
    # Ensure we're in a git repo for most commands
    set git_commands sync status diff gemini-test gemini-debug
    set command $argv[1]
