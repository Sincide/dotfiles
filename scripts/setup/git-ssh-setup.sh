...
create_ssh_config() {
    ...
    if [[ -f "$config_file" ]] && grep -q "# Generated by $SCRIPT_NAME" "$config_file"; then
        log_info "SSH config already contains entries from this script"
        
        # Check if specific host entries need updating
        local update_needed=false
        
        if [[ -n "$GITHUB_USERNAME" ]] && ! grep -q "Host github.com" "$config_file"; then
            update_needed=true
        fi
        
        if [[ -n "$GITLAB_USERNAME" ]] && ! grep -q "Host gitlab.com" "$config_file"; then
            update_needed=true
        fi
        
        if [[ "$update_needed" == "false" ]]; then
            log_info "All required SSH configurations already exist"
            return 0
        fi
    fi
    
    # Create or append to SSH config only if needed
    if [[ ! -f "$config_file" ]] || ! grep -q "# Generated by $SCRIPT_NAME" "$config_file"; then
        cat >> "$config_file" << EOF
...
EOF
    ...
    # Add GitHub configuration if username provided and not already present
    if [[ -n "$GITHUB_USERNAME" ]] && ! grep -q "Host github.com" "$config_file"; then
        cat >> "$config_file" << EOF
...
EOF
    ...
    # Add GitLab configuration if username provided and not already present
    if [[ -n "$GITLAB_USERNAME" ]] && ! grep -q "Host gitlab.com" "$config_file"; then
        cat >> "$config_file" << EOF
...
EOF
    ...
    # Set permissions and report status
    chmod 600 "$config_file"
    
    if [[ "$config_modified" == "true" ]]; then
        log_success "SSH config file updated: $config_file"
    else
        log_info "SSH config file unchanged: $config_file"
    fi
}
